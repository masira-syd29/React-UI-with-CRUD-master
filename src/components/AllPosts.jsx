  ...
  navigate("/edit",{state:{id}});
    ...
    useEffect(() => {
      const fetchPosts = async () => {
        const response = await axios.get(`http://localhost:8085/jobPosts/keyword/${query}`);
        setPost(response.data);
      };
        const fetchInitialPosts = async () => {
            const response = await axios.get(`http://localhost:8085/jobPosts`);
            setPost(response.data);
        }
         fetchInitialPosts();
      ...
      const handleDelete = (id) => {
        async function deletePost() {
          await axios.delete(`http://localhost:8085/jobPost/${id}`);
      }
      deletePost();
